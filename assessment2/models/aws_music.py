...
MUSIC_TABLE = dynamodb.Table('music1')
...
def scan_music(artist: str, title: str, year: str):
    ...
    if artist:
        filter_list.append(Attr('artist').contains(artist))
    if title:
        filter_list.append(Attr('title').contains(title))
    if year:
        filter_list.append(Attr('year').contains(year))
